...
exports.getOne = (req, res) => {
    ...
    Expert.findOne({
        ...
        if (error) throw error;
        if(!expert) res.status(ExpertMessages.error.e1.http).send(ExpertMessages.error.e1);
...
exports.update = (req, res) => {
    ...
    Expert.findOneAndUpdate({
        ...
        if (error) throw error;
        if(!expert) res.status(ExpertMessages.error.e0.http).send(ExpertMessages.error.e0);
